...
package finds where it is in the extracted text, as it does for the model's own quotes.
...
from doc_ai_kit.dataset import DatasetError, LabelRecord
from doc_ai_kit.extract import doc_id_from_name
from doc_ai_kit.normalize import LEGAL_SUFFIXES, boolean, is_null, strip_wrapping_quotes
from doc_ai_kit.normalize import date as normalize_date
from doc_ai_kit.normalize import enum as normalize_enum
from doc_ai_kit.normalize import numeric as normalize_numeric
...
FORMAT_HINTS: dict[TaskType, str] = {
    ...
    TaskType.SPAN: (
        "Paste the passage, copied from the text file. The package finds its position. "
...
def load_plan(path: Path) -> LabelPlan:
    ...
    if not path.exists():
        raise LabelingError(f"{path} does not exist; run `doc-ai-kit sample-labels` first")
...
def draw_sample(doc_ids: Sequence[str], count: int, holdout_share: float, seed: int) -> LabelPlan:
    ...
    if not corpus:
        raise LabelingError("the corpus is empty; run `doc-ai-kit extract` first")
...
def write_sheet(
    ...
    for column, header in enumerate(headers, start=1):
        ...
        if header in task_ids:
            ...
            cell.comment = Comment(f"{task.question.strip()}\n\n{FORMAT_HINTS[TaskType(task.type)]}", "doc-ai-kit")
...
def read_sheet(path: Path, registry: Registry) -> list[SheetRow]:
    ...
    """
    if not path.exists():
        raise LabelingError(f"{path} does not exist; run `doc-ai-kit label-sheet` first")
...
def import_rows(
    ...
    for row in rows:
        ...
        if text is None:
            result.problems.append(f"{row.doc_id}: no cached text; run `doc-ai-kit extract`")
            continue
        labels: dict[str, Any] = {}
