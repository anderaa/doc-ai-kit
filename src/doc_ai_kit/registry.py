...
editing the package. Validation is strict and loud: a typo in a task type must not silently
...
from doc_ai_kit.values import Granularity, PartialDate, Quantity

logger = logging.getLogger(__name__)

# appended to every span task's question. Counting characters is where a model goes wrong, so it
# is never asked to; it quotes the passage and the package finds the offsets itself
...
class TaskType(StrEnum):
    """The task types the package knows how to score."""
