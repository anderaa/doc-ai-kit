...
model and the reflection model. A package that silently picks a model for you is a package
...
class ModelConfig(_Strict):
    ...
    # in this package's own runs was a reply that spent its whole budget thinking and never
    ...
    def require_task(self) -> str:
        ...
        if not self.task:
            raise ConfigError("config.yaml: models.task is not set; the package will not choose a model for you")
        return self.task
...
class TranscriptionConfig(_Strict):
    ...
    never needs it, and a package that picks a model for you is one that spends money for you.
    """
    ...
    def _reject_removed_ocr_settings(cls, data: Any) -> Any:
        ...
        if isinstance(data, Mapping):
            ...
            if removed:
                ...
                raise ValueError(
                    "Tesseract OCR was removed in doc-ai-kit 0.1.7; pages without a text layer are now "
...
def add_excluded_classes(path: Path, additions: Mapping[str, Sequence[str]]) -> dict[str, list[str]]:
    ...
    Written by the package rather than printed for a human to paste: make-splits can produce
