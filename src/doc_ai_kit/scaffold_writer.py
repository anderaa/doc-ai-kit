...
package pin the project will install. That is why it is a console script rather than a
...
# where the package is published. A project pins one exact version of it, and that pin has
# to actually resolve: an unpublishable `doc-ai-kit==X.Y.Z` fails at the project's first
# `make sync`, which is the worst possible moment to discover it.
PACKAGE_REPO = "https://github.com/anderaa/doc-ai-kit.git"
...
class ScaffoldOptions:
    ...
    # how the project refers to the package: a git tag, a package index, or a local checkout
    pin_mode: str = "git"
    repo: str = PACKAGE_REPO
    ...
    def __post_init__(self) -> None:
        ...
        if self.pin_mode == "path" and self.package_path is None:
            raise ScaffoldError("pin mode 'path' needs --package-path pointing at a package checkout")

    @property
    def pin(self) -> str:
        """Return the dependency line the project pins the package with.
        ...
        if self.pin_mode == "pypi":
            return f"doc-ai-kit=={self.package_version}"
        if self.pin_mode == "path":
            assert self.package_path is not None  # guaranteed by __post_init__
            return f"doc-ai-kit @ file://{self.package_path.resolve()}"
        return f"doc-ai-kit @ git+{self.repo}@v{self.package_version}"
    ...
    def project_slug(self) -> str:
        ...
        return slug or "doc-ai-kit-project"
...
def create_project(target: Path, options: ScaffoldOptions, force: bool = False) -> Path:
    ...
    source = resources.files("doc_ai_kit") / "scaffold"
    ...
    logger.info("created project %s pinned to doc-ai-kit %s", target, options.package_version)
    return target
