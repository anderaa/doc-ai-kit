...
1. **The holdout opens once.** `doc-ai-kit holdout` writes a lock. A second evaluation
...
2. **`data/` is read-only to every optimization path.** `labels.jsonl` and `splits.json`
   are ground truth. The package verifies they are unchanged around every optimizer run.
...
4. **Never edit the installed `doc_ai_kit` package.** Project-specific normalizers,
   matchers and extractors go in `custom/` and register through hooks. If something cannot
   be done from `custom/`, that is a bug in the package worth reporting, not a file to patch.
